...
import Quotation from "../../models/Quotation.mjs";
const ADMIN = {
    name: "VEROA STUDIOS",
...
export const downloadQuotation = async (req, res) => {
    try {
        ...
        res.setHeader(
            "Content-Disposition",
            `attachment; filename=Quotation-${quotation.veroaBookingId}.pdf`
        );
        ...
        /* ========= INVOICE META ========= */
        doc
            .text(`Booking ID: ${quotation.veroaBookingId}`, 50, 200)
            .text(`Invoice Date: ${new Date().toLocaleDateString("en-IN")}`)
            .text(`Payment Mode: ${quotation.paymentMode}`)
            .text(`Payment Status: ${quotation.paymentStatus}`);
        ...
        doc
            .fontSize(10)
            .text(quotation.client_id?.username || "Client")
            .text(
                `${quotation.flatOrHouseNo}, ${quotation.streetName}, ${quotation.city}`
            )
            .text(`${quotation.state} - ${quotation.postalCode}`);
