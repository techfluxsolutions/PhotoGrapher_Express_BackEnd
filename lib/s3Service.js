...
export const s3Service = {
    ...
    /**
     * Generate a presigned GET URL for an existing file
     */
    ...
    getBatchSignedUrls: async (keys, expires = 3600, disposition = 'inline') => {
        const urlPromises = keys.map(key => {
            // Determine if it's a video to set appropriate headers
            const isVideo = key.match(/\.(mp4|webm|ogg|mov)$/i);

            const command = new GetObjectCommand({
                Bucket: process.env.AWS_BUCKET,
                Key: key,
                // 'inline' ensures the browser tries to play it, 'attachment' forces download
                ResponseContentDisposition: disposition,
                // Optional: Help the browser by explicitly stating the type
                ResponseContentType: isVideo ? `video/${key.split('.').pop()}` : undefined
            });
